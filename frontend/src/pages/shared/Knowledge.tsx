...
export default function Knowledge() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* Header Banner */}
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-cyan-600 via-blue-700 to-indigo-700 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Bookmark className={`w-4 h-4 ${onlyBookmarked ? 'fill-slate-900' : ''}`} />
            <span>{onlyBookmarked ? 'Showing Saved' : 'View Saved'}</span>
          </button>
        </div>
      </div>
        ...
        {filteredResources.map(res => (
          ...
          >
            <div>
              ...
              <h3 className="text-base font-black text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors line-clamp-2 mb-2">
                {res.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {res.tags.map(tag => (
                  <span key={tag} className="text-[10px] font-mono text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                ...
                <span className="font-mono text-[10px]">{res.fileSize} · {res.readTime}</span>
              </div>

              <button
                onClick={() => alert(`Opening resource: "${res.title}" (${res.format})`)}
                className="w-full py-2.5 px-3 rounded-xl bg-slate-100 hover:bg-blue-600 hover:text-white dark:bg-slate-800 dark:hover:bg-blue-600 dark:hover:text-white text-blue-600 dark:text-blue-400 font-bold text-xs flex items-center justify-center gap-1.5 transition-all"
              >
                <Download className="w-3.5 h-3.5" />
                <span>Access Document ({res.format})</span>
              </button>
            </div>
          </div>
        ))}
