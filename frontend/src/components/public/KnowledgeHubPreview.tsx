...
export const KnowledgeHubPreview: React.FC = () => {
  return (
    <section id="knowledge" className="py-20 bg-slate-950 text-white border-b border-slate-850">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-14">
          ...
          <Link
            to="/knowledge"
            className="px-5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs font-bold text-cyan-400 hover:text-cyan-300 transition-all flex items-center gap-1.5 self-start md:self-end"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {knowledgeResources.map(res => (
            <div
              key={res.id}
              className="p-5 rounded-3xl bg-slate-900 border border-slate-800 hover:border-slate-700 hover:shadow-xl transition-all flex flex-col justify-between group hover:-translate-y-1"
            >
              <div>
                <div className="flex items-center justify-between mb-3 text-xs">
                  <span className="text-2xl">{res.icon}</span>
                  <span className="px-2.5 py-0.5 rounded-md bg-slate-800 text-[10px] text-slate-300 font-bold">
                    {res.type}
                  </span>
                </div>

                <h3 className="font-extrabold text-sm text-white group-hover:text-cyan-300 transition-colors line-clamp-2 mb-2">
                  {res.title}
                </h3>
